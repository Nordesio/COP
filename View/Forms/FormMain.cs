...
namespace View
{
    public partial class FormMain : Form
    {
        ...
        private void CreateImageDoc()
        {
            ...
            {
                if (_plugins[_selectedPlugin].CreateImageDoc(new
        ...
        private void CreateTableDoc()
        {
            using (var dialog = new SaveFileDialog { Filter = "pdf|*.pdf" })
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    if (_plugins[_selectedPlugin].CreateTableDoc(new
                    PluginsConventionSaveDocument
                    { FileName = dialog.FileName }))
                    {
                        MessageBox.Show("Документ сохранен", "Создание документа",
        ...
        private void CreateChartDoc()
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                if (_plugins[_selectedPlugin].CreateChartDoc(new
                PluginsConventionSaveDocument
                { FileName = dialog.FileName }))
                {
                    MessageBox.Show("Документ сохранен", "Создание документа",
        ...
        private void ImageDocToolStripMenuItem_Click(object sender, EventArgs e) =>
        CreateImageDoc();
